...
#define I2C_BL_NAME                              "lm3532"
#define MAX_BRIGHTNESS_LM3532                    0xFF
#define MIN_BRIGHTNESS_LM3532                    0x0F
...
static struct i2c_client *lm3532_i2c_client;
...
static const struct i2c_device_id lm3532_bl_id[] = {
	{ I2C_BL_NAME, 0 },
	{ },
};
#if defined(CONFIG_LGE_BACKLIGHT_CABC)
static int lm3532_read_reg(struct i2c_client *client, u8 reg, u8 *buf);
#endif // CABC apply

static int lm3532_write_reg(struct i2c_client *client, unsigned char reg, unsigned char val);
...
static struct lm3532_device *main_lm3532_dev;
...
static void lm3532_set_main_current_level(struct i2c_client *client, int level)
{
	struct lm3532_device *dev;
	dev = (struct lm3532_device *)i2c_get_clientdata(client);
	...
	mutex_lock(&main_lm3532_dev->bl_mutex);
	if (level != 0) {
		cal_value = mapped_value[level];
		lm3532_write_reg(client, 0x70, cal_value); // Control A Zone Target Register == brightness
	} else {
		lm3532_write_reg(main_lm3532_dev->client, 0x1d, 0x00);	/* Disable LED A */
	}
	mutex_unlock(&main_lm3532_dev->bl_mutex);

       printk(KERN_INFO "[LCD][DEBUG] %s : backlight level=%d, cal_value=%d\n", __func__, level, cal_value);
}

static void lm3532_set_main_current_level_no_mapping(struct i2c_client *client, int level)
{
	struct lm3532_device *dev;
	dev = (struct lm3532_device *)i2c_get_clientdata(client);
	...
	mutex_lock(&main_lm3532_dev->bl_mutex);
	if (level != 0) {
		lm3532_write_reg(client, 0x70, level);
	} else {
		lm3532_write_reg(main_lm3532_dev->client, 0x1d, 0x00);	/* Disable LED A */
	}
	mutex_unlock(&main_lm3532_dev->bl_mutex);
...
void lm3532_backlight_on(int level)
{
	...
	if (backlight_status == BL_OFF) {
		...
		lm3532_write_reg(main_lm3532_dev->client, 0x1d, 0x01);	/* Enable LED A */
#if defined(CONFIG_LGE_BACKLIGHT_CABC)
		lm3532_write_reg(main_lm3532_dev->client, 0x13, 0x06);	/*  Active PWM input is enabled in Zone 0, active high polarity, PWM2 is mapped to Control Bank A*/
#else
		lm3532_write_reg(main_lm3532_dev->client, 0x16, 0x03);	/*  Control A Zone Target 0, Linear Mapping, I2C Current Control */
#endif // CABC apply
		lm3532_write_reg(main_lm3532_dev->client, 0x17, 0x10);  /* Full-Scale Current (17.8mA) of BANK A */
	}
	mdelay(1);   /* udelay(100); */

	lm3532_set_main_current_level(main_lm3532_dev->client, level);
...
#if !defined(CONFIG_HAS_EARLYSUSPEND)
void lm3532_backlight_off(void)
#else
void lm3532_backlight_off(struct early_suspend * h)
#endif /* CONFIG_HAS_EARLYSUSPEND */
{
	int gpio = main_lm3532_dev->gpio;
	...
	lm3532_set_main_current_level(main_lm3532_dev->client, 0);
...
void lm3532_lcd_backlight_set_level(int level)
{
	if (level > MAX_BRIGHTNESS_LM3532)
		level = MAX_BRIGHTNESS_LM3532;

//	printk("### %s level = (%d) \n ",__func__,level);
	if (lm3532_i2c_client != NULL) {
		if (level == 0) {
#if !defined(CONFIG_HAS_EARLYSUSPEND)
			lm3532_backlight_off();
#else
			lm3532_backlight_off(h);
#endif /* CONFIG_HAS_EARLYSUSPEND */
		} else {
			lm3532_backlight_on(level);
		}
	} else {
		pr_err("%s(): No client\n", __func__);
	}
}
EXPORT_SYMBOL(lm3532_lcd_backlight_set_level);

#ifdef CONFIG_HAS_EARLYSUSPEND
void lm3532_early_suspend(struct early_suspend * h)
{
	pr_info("%s[Start] backlight_status: %d\n", __func__, backlight_status);
	if (backlight_status == BL_OFF)
		return;

	lm3532_lcd_backlight_set_level(0);
	is_early_suspended = true;
	return;
}

void lm3532_late_resume(struct early_suspend * h)
{
	pr_info("%s[Start] backlight_status: %d\n", __func__, backlight_status);
	...
	lm3532_lcd_backlight_set_level(saved_main_lcd_level);
#endif
...
static int bl_set_intensity(struct backlight_device *bd)
{
	...
	lm3532_set_main_current_level(client, bd->props.brightness);
...
static ssize_t lcd_backlight_store_level(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	level = simple_strtoul(buf, NULL, 10);
       ...
       lm3532_set_main_current_level_no_mapping(client, level);
       printk(KERN_INFO "[LCD][DEBUG] write %d direct to backlight register\n", level);

	return count;
}

static int lm3532_bl_resume(struct i2c_client *client)
{
	lm3532_lcd_backlight_set_level(saved_main_lcd_level);
       return 0;
}

static int lm3532_bl_suspend(struct i2c_client *client, pm_message_t state)
{
...
static ssize_t lcd_backlight_store_on_off(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (on_off == 1) {
		lm3532_bl_resume(client);
	} else if (on_off == 0)
	    lm3532_bl_suspend(client, PMSG_SUSPEND);
...
static ssize_t lcd_backlight_show_pwm(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&main_lm3532_dev->bl_mutex);
	lm3532_read_reg(main_lm3532_dev->client, 0x01, &config);
	mdelay(3);
	lm3532_read_reg(main_lm3532_dev->client, 0x03, &level);
	mdelay(3);
	lm3532_read_reg(main_lm3532_dev->client, 0x12, &pwm_low);
	mdelay(3);
	lm3532_read_reg(main_lm3532_dev->client, 0x13, &pwm_high);
	mdelay(3);
	mutex_unlock(&main_lm3532_dev->bl_mutex);
...
DEVICE_ATTR(lm3532_level, 0644, lcd_backlight_show_level, lcd_backlight_store_level);
DEVICE_ATTR(lm3532_backlight_on_off, 0644, lcd_backlight_show_on_off, lcd_backlight_store_on_off);
DEVICE_ATTR(lm3532_exp_min_value, 0644, lcd_backlight_show_exp_min_value, lcd_backlight_store_exp_min_value);
DEVICE_ATTR(lm3532_shaking_delay, 0644, lcd_backlight_show_shaking_delay, lcd_backlight_store_shaking_delay);
#if defined(CONFIG_LGE_BACKLIGHT_CABC)
DEVICE_ATTR(lm3532_pwm, 0644, lcd_backlight_show_pwm, lcd_backlight_store_pwm);
#endif // CABC apply

static struct backlight_ops lm3532_bl_ops = {
	.update_status = bl_set_intensity,
	.get_brightness = bl_get_intensity,
};

static int lm3532_probe(struct i2c_client *i2c_dev, const struct i2c_device_id *id)
{
	...
	lm3532_i2c_client = i2c_dev;

	dev = kzalloc(sizeof(struct lm3532_device), GFP_KERNEL);
	if (dev == NULL) {
		dev_err(&i2c_dev->dev, "fail alloc for lm3532_device\n");
		return 0;
	}

	printk(KERN_INFO "[LCD][DEBUG] %s: gpio = %d\n", __func__,pdata->gpio);

	if (pdata->gpio && gpio_request(pdata->gpio, "lm3532 reset") != 0) {
		return -ENODEV;
	}
	...
	props.max_brightness = MAX_BRIGHTNESS_LM3532;
	bl_dev = backlight_device_register(I2C_BL_NAME, &i2c_dev->dev, NULL, &lm3532_bl_ops, &props);
	bl_dev->props.max_brightness = MAX_BRIGHTNESS_LM3532;
	...
	err = device_create_file(&i2c_dev->dev, &dev_attr_lm3532_level);
	err = device_create_file(&i2c_dev->dev, &dev_attr_lm3532_backlight_on_off);
	err = device_create_file(&i2c_dev->dev, &dev_attr_lm3532_exp_min_value);
	err = device_create_file(&i2c_dev->dev, &dev_attr_lm3532_shaking_delay);
#if defined(CONFIG_LGE_BACKLIGHT_CABC)
    err = device_create_file(&i2c_dev->dev, &dev_attr_lm3532_pwm);
#endif // CABC apply

#ifdef CONFIG_HAS_EARLYSUSPEND
	early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	early_suspend.suspend = lm3532_early_suspend;
	early_suspend.resume = lm3532_late_resume;
	register_early_suspend(&early_suspend);
#endif /* CONFIG_HAS_EARLYSUSPEND */
//       lm3532_backlight_on(250);
	return 0;
}

static int lm3532_remove(struct i2c_client *i2c_dev)
{
	struct lm3532_device *dev;
	int gpio = main_lm3532_dev->gpio;

	device_remove_file(&i2c_dev->dev, &dev_attr_lm3532_level);
	device_remove_file(&i2c_dev->dev, &dev_attr_lm3532_backlight_on_off);
	dev = (struct lm3532_device *)i2c_get_clientdata(i2c_dev);
...
static struct i2c_driver main_lm3532_driver = {
	.probe = lm3532_probe,
	.remove = lm3532_remove,
	.suspend = NULL,
	.resume = NULL,
	.id_table = lm3532_bl_id,
	.driver = {
		.name = I2C_BL_NAME,
		.owner = THIS_MODULE,
	},
...
static int __init lcd_backlight_init(void)
{
	...
	err = i2c_add_driver(&main_lm3532_driver);
...
MODULE_DESCRIPTION("LM3532 Backlight Control");
